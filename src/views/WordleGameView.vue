<template>
  <section class="wordly">
      ...
      <main class="wd-center" @click="maybeFocusMobile" aria-label="Game board">
        <div class="wd-board-col">
          <div class="wd-board" :style="{ gridTemplateColumns: `repeat(${targetLen}, var(--cell))` }">
            <template v-for="r in maxAttempts" :key="r">
              <div v-for="c in targetLen" :key="`${r}-${c}`" class="wd-cell"
                   :class="cellClass(r-1, c-1)" :style="flipStyle(r-1, c-1)">
                {{ letterAt(r-1, c-1) }}
              </div>
            </template>
          </div>
          <input ref="mobileInput" class="wd-hidden-input" inputmode="latin" autocomplete="off"
                 autocapitalize="off" spellcheck="false" @keydown.prevent="onKeydown" />
        </div>
        ...
        <div class="wd-mobile-panels">
          <details class="wd-coll">
            ...
            <ol class="wd-steps">
              <li>Guess the word in <strong>{{ maxAttempts }}</strong> tries.</li>
              <li>Each guess must be a valid <strong>{{ targetLen }}</strong>-letter word.</li>
            </ol>
          </details>
          <details class="wd-coll" style="margin-top:10px">
            <summary>Rules & Tips</summary>
            <ul class="wd-bullets">
              <li>Duplicates can appear.</li>
            </ul>
          </details>
        </div>
    ...
    <!-- FAB + Bottom Sheet (mobile/narrow) -->
    <button class="tips-fab" @click="mobileTipsOpen = true" aria-label="Health Tips">Health Tips</button>

    <div class="tips-drawer" :class="{ open: mobileTipsOpen }" role="dialog" aria-label="Health Tips (mobile)">
      <header class="drawer-h">
        <span class="drawer-title">Health Tips</span>
        <button class="x" @click="mobileTipsOpen = false" aria-label="Close">×</button>
      </header>
...
const REVEAL_GAP = 140, SINGLE_FLIP = 250, HALF_FLIP = Math.floor(SINGLE_FLIP/2);

/* keyboard states */
const keyState = reactive(Object.fromEntries('abcdefghijklmnopqrstuvwxyz'.split('').map(ch => [ch, ''])));
function updateKeyState(letter, st){
  const curSt = keyState[letter];
  if (curSt === 'correct') return;
  if (curSt === 'present' && st === 'absent') return;
  keyState[letter] = st;
}

/* confetti */
const confettiCanvas = ref(null); let confettiTimer = null; const confettiRunning = ref(false);
...
/* confetti */
function resizeCanvas(){ const cvs = confettiCanvas.value; if (!cvs) return; cvs.width=window.innerWidth; cvs.height=window.innerHeight; cvs.style.background='transparent'; }
async function launchConfetti(){
  ...
  const ctx = cvs.getContext('2d'); const particles = Array.from({length:200}).map(()=>({
    x:Math.random()*cvs.width, y:-20-Math.random()*120, r:2+Math.random()*4, vx:-1+Math.random()*2, vy:2+Math.random()*3.5, a:Math.random()*Math.PI*2, va:-0.25+Math.random()*0.5
  }));
  ...
  function frame(t){
    ...
    for (const p of particles){
      ...
      const hue=(p.x/cvs.width)*360; ctx.fillStyle=`hsl(${hue},90%,60%)`;
...
/* ===== grid ===== */
.wd-stage{
  display:grid; grid-template-columns: 300px minmax(420px,1fr) 320px; column-gap:28px; align-items:start;
}
...
@media (max-width:1280px){ .wd-right-col{ display:none; } }

/* mobile layout */
@media (max-width:980px){
  .wd-stage{ grid-template-columns: 1fr; }
  .wd-left-stack{ display:none; }
  .wordly{ --cell:46px; }
  .wd-key{ padding:8px 10px; }
  .wd-mobile-panels{ display:block; margin-top:12px; }
  .wd-coll{ display:block; background:#10121a; border:1.5px solid #50536b; border-radius:12px; padding:10px 12px; box-shadow:0 0 0 1px rgba(80,83,107,.08) inset; }
  ...
  .wd-coll > summary::before{ content:'▸'; transform:translateY(1px); opacity:.9; }
...
.tips-fab{
  ...
  background:#4f46e5; color:#fff; border:0; padding:10px 14px; border-radius:999px; font-weight:800;
  display:none; box-shadow:0 6px 24px rgba(79,70,229,.35);
}
@media (max-width:1280px){ .tips-fab{ display:inline-flex; } }
...
.drawer-h .x{ position:absolute; right:10px; top:6px; width:30px; height:30px; border-radius:8px; border:1px solid #37415b; background:#1a2032; color:#e6e6eb; }
.drawer-body{ padding:12px; overflow:auto; max-height:calc(90dvh - 54px); }
</style>
